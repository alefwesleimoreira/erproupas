...
export default function Orders() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/orders"] }); toast({ title: "Sucesso", description: "Status atualizado" }); },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <div className="space-y-4">{[...Array(4)].map((_, i) => <Card key={i}><CardHeader><Skeleton className="h-6 w-48" /></CardHeader></Card>)}</div> : filteredOrders?.length ? <div className="space-y-4">{filteredOrders.map((order) => <Card key={order.id}><CardHeader><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="flex items-center gap-3"><span>{order.customerName}</span><Badge className={`${statusConfig[order.status].bgClass} ${statusConfig[order.status].textClass}`}>{statusConfig[order.status].label}</Badge></CardTitle><CardDescription>Pedido #{order.id.slice(0, 8)} • {format(new Date(order.orderDate), "dd/MM/yyyy HH:mm")}</CardDescription></div><div className="flex items-center gap-2"><span className="text-lg font-bold">{formatCurrency(order.totalAmount)}</span><Select value={order.status} onValueChange={(value) => updateStatusMutation.mutate({ id: order.id, status: value as OrderStatus })}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent>{orderStatuses.map((status) => <SelectItem key={status} value={status}>{statusConfig[status].label}</SelectItem>)}</SelectContent></Select><Dialog><DialogTrigger asChild><Button variant="outline" size="icon"><Eye className="h-4 w-4" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Detalhes do Pedido</DialogTitle><DialogDescription>Pedido #{order.id.slice(0, 8)}</DialogDescription></DialogHeader><div className="space-y-2 text-sm"><p><strong>Cliente:</strong> {order.customerName}</p><p><strong>Telefone:</strong> {order.customerPhone}</p><p><strong>Endereço:</strong> {order.customerAddress}</p><div className="pt-2 border-t">{order.items.map((item) => <div key={item.id} className="flex justify-between"><span>{item.productName} x {item.quantity}</span><span>{formatCurrency(item.quantity * item.unitPrice)}</span></div>)}</div></div></DialogContent></Dialog></div></div></CardHeader></Card>)}</div> : <Card><CardContent className="py-16 text-center"><ShoppingCart className="mx-auto h-16 w-16 text-muted-foreground" /><h3 className="mt-4 text-lg font-semibold text-foreground">Nenhum pedido encontrado</h3></CardContent></Card>}
    </div>
  );
}
